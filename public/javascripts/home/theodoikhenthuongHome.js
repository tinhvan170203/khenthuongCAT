$(document).ready(function() {
    ...
    $('#form-filter').on('submit', function(e) {
        ...
        $.ajax({
            url: `/pm/thiduakhenthuong/filterKhenthuong`,
            ...
            success: function(data) {
                ...
                if (data.tongsophongduockhenthuong.length > 0) {
                    $('#khenthuong-list').append(`
                        <tr  class="sub-head">
                            <th colspan="5">Tập thể cấp phòng và tương đương trở lên: ${data.tongsophongduockhenthuong.length} đơn vị</th>
                        </tr>         
                    `)
                    data.tongsophongduockhenthuong.forEach((item, index) => {
                        ...
                        <tr>
                            <td>${index+1}</td>
                            <td colspan="4">${item.kyhieu} - ${item.soluot} lượt </td>
                        </tr>     
                        `)
                    })
                };
