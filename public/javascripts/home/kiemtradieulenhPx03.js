$(document).ready(function() {
            ...
            result.forEach((item, index) => {
                $('#bang2').append(`
                          <tr>
                                  ...
                                  <td>${item.donviduockiemtra}</td>
                                  <td>${item.thoigian.slice(11)} ${item.thoigian.slice(0,10).split('-').reverse().join('/')}</td>
                                  <td>${item.ketqua}</td>
                                  <td>${item.xuli}</td>
                                  <td style="text-align: center"><a class="download" href="/congtacdieulenh/kiemtra/download/${item.tep}"><i style="font-size:20px" class="fas fa-file-word"></i></a></td>
                          </tr>
                          `)
            });
        }
    ...
    $('#form-filter').on('submit', function(e) {
        ...
        $.ajax({
            url: `/pm/thiduakhenthuong/filterKiemtraPx03`,
            data: { donviduockiemtra, ketqua, xuli, ngay_start, ngay_end },
