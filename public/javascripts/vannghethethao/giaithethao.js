$(document).ready(function() {
    ...
    $("#ketquaGiai").click(function(e) {
        ...
        $('#quantri-detail').html('');
        if (allRole.indexOf('them-ket-qua-the-thao') !== -1) {
        ...
        $('#quantri-detail').append(`
            <h4 class="quantri-detail-head">Kết quả tham gia các giải thi đấu thể thao do các cấp tổ chức</span>
            </h4>
            <table id="tableGiaithethao" style="width:100%" class="table table-bordered table-striped">
                <thead>
                    <tr>
                        <th style="text-align: center;">STT</th>
                        <th style="text-align: center;">Tên giải thi đấu</th>
    ...
    $('#cuocthiForm').on('submit', function(e) {
        ...
        $.ajax({
            url: '/vannghethethao/quantri/thethao/ketqua/giaithethao/add',
            method: 'POST',
            data: { tengiai, donvitochuc, ngaytochuc, ketqua, diadiemtochuc },
            error: function() {
                alert('Không có quyền thêm mới kết quả giải thể thao hoặc có lỗi xảy ra trong quá trình thêm mới...')
            },
    ...
    const tableGiaiThethao = () => {
        table = $('#tableGiaithethao').DataTable({
            ajax: {
                url: '/vannghethethao/quantri/thethao/ketqua/giaithethao',
                dataSrc: ''
            },
            "lengthMenu": [15],
            select: true,
            ...
            "aoColumns": [{
                ...
                },
                {
                    "mData": function(data, type, dataToSet) {
                        let dataRender = '';
                        if (allRole.indexOf('sua-ket-qua-the-thao') !== -1) {
                ...
                },
                {
                    "mData": function(data, type, dataToSet) {
                        let dataRender = '';
                        if (allRole.indexOf('xoa-ket-qua-the-thao') !== -1) {
    ...
    $('#EditCuocthiForm').on('submit', function(e) {
        ...
        $.ajax({
            url: '/vannghethethao/quantri/thethao/ketqua/giaithethao/edit/' + idCuocthiEdit,
            method: 'POST',
            data: { tengiai, donvitochuc, ngaytochuc, ketqua, diadiemtochuc },
            error: function() {
                alert('Có lỗi xảy ra...')
    ...
    $(document).on('click', '.deleteCuocthi', function() {
        ...
        if (isDelete) {
            $.ajax({
                url: '/vannghethethao/quantri/thethao/ketqua/giaithethao/delete/' + idCuocthiDelete,
                method: 'GET',
                error: function() {
                    alert('Có lỗi xảy ra...')
                },
    ...
    //Quản trị tên môn thể thao thao
    $("#thethaoList").click(function(e) {
        ...
        parentNode.addClass('active-tab');
        $('#quantri-detail').html('');
        if (allRole.indexOf('them-mon-the-thao') !== -1) {
            $('#quantri-detail').append(`
        ...
        $('#quantri-detail').append(`
            <h4 class="quantri-detail-head">Quản trị các môn thể thao có các nhân tố điển hình nổi bật</span>
            </h4>
            <table id="tableThethaoList" style="width:100%" class="table table-bordered table-striped">
                <thead>
                    <tr>
                        <th style="text-align: center;">STT</th>
                        <th style="text-align: center;">Tên môn thể thao</th>
                        <th style="text-align: center;"></th>
                        <th style="text-align: center;"></th>
                    </tr>
                </thead>
    ...
    const tableThethaoList = () => {
        tableInitThethaoList = $('#tableThethaoList').DataTable({
            ajax: {
                url: '/vannghethethao/quantri/thethao/thethaoList',
                dataSrc: ''
            },
            "lengthMenu": [6],
            select: true,
            ...
            "aoColumns": [{
                ...
                },
                {
                    "mData": function(data, type, dataToSet) {
                        let dataRender = '';
                        if (allRole.indexOf('sua-mon-the-thao') !== -1) {
    ...
    $('#sportForm').on('submit', function(e) {
        ...
        $.ajax({
            url: `/vannghethethao/quantri/thethao/monthethao/add`,
            method: 'POST',
            data: { monthethao },
            error: function() {
                alert('Bạn không có quyền thêm môn thể thao hoặc có lỗi hệ thống xảy ra')
            },
            success: function(data) {
                if (data === "Tài khoản đã bị xóa bởi Admin hệ thống.") {
                    window.location.href = `/quantrihethong/checkRole/error/${data}`
    ...
    $('#sportFormEdit').on('submit', function(e) {
        ...
        $.ajax({
            url: `/vannghethethao/quantri/thethao/monthethao/edit/${idMonThethao}`,
            method: 'POST',
            data: { monthethao },
            error: function() {
                alert('Bạn không có quyền sửa môn thể thao hoặc có lỗi xảy ra')
            },
            success: function(data) {
                if (data === "Tài khoản đã bị xóa bởi Admin hệ thống.") {
                    window.location.href = `/quantrihethong/checkRole/error/${data}`
    ...
    $(document).on('click', '.deleteMonThethao', function() {
        ...
        let isDelete = confirm('Bạn có muốn xóa môn thể thao này?')
        if (isDelete) {
            $.ajax({
                url: `/vannghethethao/quantri/thethao/monthethao/delete/${idDeleteSport}`,
                error: function() {
                    alert('Bạn không có quyền xóa môn thể thao hoặc có lỗi xảy ra')
                },
                success: function(data) {
                    if (data === "Tài khoản đã bị xóa bởi Admin hệ thống.") {
                        window.location.href = `/quantrihethong/checkRole/error/${data}`
    ...
    $("#starCanhan").click(function(e) {
        ...
        $('#quantri-detail').html('');
        $('#quantri-detail').append(`
                    <h4 class="quantri-detail-head">Các cá nhân, tập thể tiêu biểu trong nội dung <span id="sportRender"></span><span id="nampage"></span></h4>
        ...
        $('#thongke').click(function() {
            if (allRole.indexOf('them-ca-nhan-the-thao-dien-hinh') !== -1) {
    ...
    const tableTieubieu = () => {
        ...
        tableInitTieubieu = $('#tableCanhanTieubieu').DataTable({
            ajax: {
                url: `/vannghethethao/quantri/thethao/canhantieubieu/${noidung}/${nam}`,
                dataSrc: ''
            },
            "lengthMenu": [50],
            "bAutoWidth": false,
            ...
            "aoColumns": [{
                ...
                "mData": function(data, type, dataToSet) {
                    let dataRender = '';
                    if (allRole.indexOf('xoa-ca-nhan-the-thao-dien-hinh') !== -1) {
    ...
    $('#tieubieuForm').on('submit', function(e) {
        ...
        $.ajax({
            url: `/vannghethethao/quantri/thethao/canhantieubieu/${noidung}/${nam}/updateList`,
            method: 'POST',
            error: function() {
                alert('có lỗi')
            },
    ...
    $(document).on('click', '.deleteCanhantieubieu', function() {
        ...
        if (isDelete) {
            $.ajax({
                url: `/vannghethethao/quantri/thethao/canhantieubieu/${noidung}/${nam}/delete/${idDelete}`,
                method: 'GET',
                error: function() {
                    alert('Có lỗi xảy ra...')
                },
