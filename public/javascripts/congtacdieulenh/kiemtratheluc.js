$(document).ready(function() {
    ...
    $("#ketquaGiai").click(function(e) {
        ...
        $('#quantri-detail').append(`
                    <div class="quantri-tab-select">
                            ...
                            `);
        if (allRole.indexOf('them-kiem-tra-the-luc') !== -1) {
            $('#quantri-detail').append(`
        ...
        $('#quantri-detail').append(`
         <h4 class="quantri-detail-head">Báo cáo kết quả kiểm tra thể lực của các đơn vị <span id="namCurrent"></span></h4>
    ...
    $('#cuocthiForm').on('submit', function(e) {
        ...
        $.ajax({
            url: '/congtacdieulenh/quantri/kiemtra/theluc/add/' + nam,
            method: 'POST',
            contentType: false,
            processData: false,
            data: fd,
    ...
    const tableTheluc = () => {
        ...
        table = $('#tableGiaithethao').DataTable({
            ajax: {
                url: '/congtacdieulenh/quantri/kiemtra/theluc/tableTheluc/' + nam,
                dataSrc: ''
            },
            "lengthMenu": [15],
            select: true,
            ...
            "aoColumns": [{
                ...
                {
                    "mData": function(data, type, dataToSet) {
                        return `<p  style="text-align:center"><a class="download"  href="/congtacdieulenh/quantri/kiemtra/theluc/download/${data.tep}"><i style="font-size:20px" class="fas fa-file-word"></i></a></p>`
                    },
                    "width": "15%"
                },
                {
                    "mData": function(data, type, dataToSet) {
                        let dataRender = '';
                        if (allRole.indexOf('sua-kiem-tra-the-luc') !== -1) {
                ...
                },
                {
                    "mData": function(data, type, dataToSet) {
                        let dataRender = '';
                        if (allRole.indexOf('xoa-kiem-tra-the-luc') !== -1) {
    ...
    $('#EditCuocthiForm').on('submit', function(e) {
        ...
        let fd = new FormData(e.target); // submit sẽ lấy tất cả giá trị qua thuộc tính name của thẻ
        $.ajax({
            url: '/congtacdieulenh/quantri/kiemtra/theluc/edit/' + idCuocthiEdit,
            method: 'POST',
            contentType: false,
            processData: false,
            data: fd,
    ...
    $(document).on('click', '.deleteCuocthi', function() {
        ...
        if (isDelete) {
            $.ajax({
                url: '/congtacdieulenh/quantri/kiemtra/theluc/delete/' + idCuocthiDelete,
                method: 'GET',
                error: function() {
                    alert('Có lỗi xảy ra...')
                },
    ...
    // Danh sách không đạt kiểm tra thể lực
    $("#starCanhan").click(function(e) {
        ...
        $('#quantri-detail').html('');
        $('#quantri-detail').append(`
                         <h4 class="quantri-detail-head">Danh sách cá nhân kiểm tra thể lực xếp loại không đạt <span id="sportRender"></span><span id="nampage"></span></h4>
                         <div class="quantri-tab-select">
    ...
    const tableTieubieu = () => {
        ...
        tableInitTieubieu = $('#tableCanhanTieubieu').DataTable({
            ajax: {
                url: `/congtacdieulenh/quantri/kiemtra/theluc/khongdat/${nam}`,
                dataSrc: ''
            },
            "lengthMenu": [10],
            select: true,
    ...
    //chọn đơn vị thêm cán bộ k đạt thể lực
    ...
    $('#tieubieuForm').on('submit', function(e) {
        ...
        $.ajax({
            url: `/congtacdieulenh/quantri/kiemtra/theluc/khongdat/${nam}/updateList`,
            method: 'POST',
            error: function() {
                alert('có lỗi')
            },
    ...
    $(document).on('click', '.deleteCanhantieubieu', function() {
        ...
        if (isDelete) {
            $.ajax({
                url: `/congtacdieulenh/quantri/kiemtra/theluc/khongdat/${nam}/delete/${idDelete}`,
                method: 'GET',
                error: function() {
                    alert('Có lỗi xảy ra...')
                },
