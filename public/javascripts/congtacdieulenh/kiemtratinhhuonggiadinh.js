$(document).ready(function() {
    ...
    $('#cuocthiForm').on('submit', function(e) {
        ...
        $.ajax({
            url: '/congtacdieulenh/quantri/kiemtra/tinhhuong/add',
            method: 'POST',
            contentType: false,
            processData: false,
            data: fd,
    ...
    const tableKetquaKTDieulenh = () => {
        ...
        table = $('#tableGiaithethao').DataTable({
            ajax: {
                url: '/congtacdieulenh/quantri/kiemtra/tinhhuong/' + nam,
                dataSrc: ''
            },
            "lengthMenu": [15],
            select: true,
            ...
            "aoColumns": [{
                ...
                {
                    "mData": function(data, type, dataToSet) {
                        return `<p class="xuli" style="text-align:center"><a class="download"  href="/congtacdieulenh/kiemtra/tinhhuong/download/${data.tep}"><i style="font-size:20px" class="fas fa-file-word"></i></a></p>`
                    },
                    "width": "0.5%"
                },
                {
                    "mData": function(data, type, dataToSet) {
                        let dataRender = '';
                        if (allRole.indexOf('sua-kiem-tra-tinh-huong-gia-dinh') !== -1) {
                ...
                },
                {
                    "mData": function(data, type, dataToSet) {
                        let dataRender = '';
                        if (allRole.indexOf('xoa-kiem-tra-tinh-huong-gia-dinh') !== -1) {
    ...
    $('#nam').change(function() {
        let nam = $(this).val();
        table.ajax.url(`/congtacdieulenh/quantri/kiemtra/tinhhuong/${nam}`).load()
    });
    ...
    $('#EditCuocthiForm').on('submit', function(e) {
        ...
        $.ajax({
            url: '/congtacdieulenh/quantri/kiemtra/tinhhuong/edit/' + idCuocthiEdit,
            method: 'POST',
            contentType: false,
            processData: false,
            data: fd,
    ...
    $(document).on('click', '.deleteCuocthi', function() {
        ...
        if (isDelete) {
            $.ajax({
                url: '/congtacdieulenh/quantri/kiemtra/tinhhuong/delete/' + idCuocthiDelete,
                method: 'GET',
                error: function() {
                    alert('Có lỗi xảy ra...')
                },
