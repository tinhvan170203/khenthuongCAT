$(document).ready(function() {
    ...
    $("#ketquaGiai").click(function(e) {
        ...
        <div class="quantri-tab-select">
                ...
                `);
        if (allRole.indexOf('them-kiem-tra-quan-su-vo-thuat') !== -1) {
            $('#quantri-detail').append(`
    ...
    `)
        ...
        $('#quantri-detail').append(`
           <h4 class="quantri-detail-head">Báo cáo kết quả kiểm tra Điều lệnh -  Quân sự - Võ thuật của các đơn vị <span id="namCurrent"></span>
    ...
    $('#cuocthiForm').on('submit', function(e) {
        ...
        $.ajax({
            url: '/congtacdieulenh/quantri/kiemtra/dieulenhquansu/add/' + nam,
            method: 'POST',
            contentType: false,
            processData: false,
            data: fd,
    ...
    const tableTheluc = () => {
        ...
        table = $('#tableGiaithethao').DataTable({
            ajax: {
                url: '/congtacdieulenh/quantri/kiemtra/dieulenhquansu/tableQuansu/' + nam,
                dataSrc: ''
            },
            "lengthMenu": [15],
            select: true,
            ...
            "aoColumns": [{
                ...
                },
                {
                    "mData": function(data, type, dataToSet) {
                        let dataRender = '';
                        if (allRole.indexOf('sua-kiem-tra-quan-su-vo-thuat') !== -1) {
                ...
                },
                {
                    "mData": function(data, type, dataToSet) {
                        let dataRender = '';
                        if (allRole.indexOf('xoa-kiem-tra-quan-su-vo-thuat') !== -1) {
    ...
    $('#EditCuocthiForm').on('submit', function(e) {
        ...
        let fd = new FormData(e.target); // submit sẽ lấy tất cả giá trị qua thuộc tính name của thẻ
        $.ajax({
            url: '/congtacdieulenh/quantri/kiemtra/dieulenhquansu/edit/' + idCuocthiEdit,
            method: 'POST',
            contentType: false,
            processData: false,
            data: fd,
    ...
    $(document).on('click', '.deleteCuocthi', function() {
        ...
        if (isDelete) {
            $.ajax({
                url: '/congtacdieulenh/quantri/kiemtra/dieulenhquansu/delete/' + idCuocthiDelete,
                method: 'GET',
                error: function() {
                    alert('Có lỗi xảy ra...')
                },
    ...
    // Danh sách không đạt quân sự, võ thuật
    ...
    const tableTieubieu = () => {
        ...
        tableInitTieubieu = $('#tableCanhanTieubieu').DataTable({
            ajax: {
                url: `/congtacdieulenh/quantri/kiemtra/dieulenhquansu/khongdat/${nam}`,
                dataSrc: ''
            },
            "lengthMenu": [10],
            select: true,
    ...
    $('#tieubieuForm').on('submit', function(e) {
        ...
        $.ajax({
            url: `/congtacdieulenh/quantri/kiemtra/dieulenhquansu/khongdat/${nam}/updateList`,
            method: 'POST',
            error: function() {
                alert('có lỗi')
            },
    ...
    $(document).on('click', '.deleteCanhantieubieu', function() {
        ...
        if (isDelete) {
            $.ajax({
                url: `/congtacdieulenh/quantri/kiemtra/dieulenhquansu/khongdat/${nam}/delete/${idDelete}`,
                method: 'GET',
                error: function() {
                    alert('Có lỗi xảy ra...')
                },
