$(document).ready(function() {
    ...
    $('#cuocthiForm').on('submit', function(e) {
        e.preventDefault();
        let id = $('#chuyenmucList').val();
        let muctin = $('#ten').val().trim();
        $.ajax({
            url: '/tuyentruyen/' + id + '/addMuctin',
            method: 'POST',
            data: { muctin },
            error: function() {
    ...
    const getCuocthiList = () => {
        ...
        table = $('#ketquaTable').DataTable({
            ...
            "aoColumns": [{
                ...
                {
                    "mData": function(data, type, dataToSet) {
                        return `<span class="ten" id="${data._id}">${data.muctin}</span>`
                    },
                    "width": "93%"
                },
                {
    ...
    $('#chuyenmucList').change(function() {
        let id = $("#chuyenmucList").val();
        table.ajax.url(`/tuyentruyen/${id}/muctin`).load(null, false);
    });
    ...
    $('#EditCuocthiForm').on('submit', function(e) {
        let id = $("#chuyenmucList").val();
        e.preventDefault();
        let muctinEdit = $('#tenEdit').val().trim();
        $.ajax({
            url: '/tuyentruyen/' + id + "/editMuctin/muctin/" + idCuocthiEdit,
            method: 'POST',
            data: { muctinEdit },
            error: function() {
    ...
    $(document).on('click', '.deleteCuocthi', function() {
        ...
        if (isDelete) {
            $.ajax({
                url: '/tuyentruyen/' + id + "/deleteMuctin/muctin/" + idCuocthiDelete,
                method: 'GET',
                error: function() {
