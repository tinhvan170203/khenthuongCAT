const id = $('.tenphong').attr('id');
const tenphong = $('.nam-thiduathang').attr('id');
...
function thiduathang(data, year) {
    Highcharts.chart('thongke-thiduathang', {
        ...
        subtitle: { //đề mục con
            text: 'Thống kê kết quả thi đua tháng trong năm ' + year + ' ' + tenphong,
            color: '#038eae',
        },
        title: {
            text: ''
...
function thiduathangRate(red, blue) {
    Highcharts.chart('rate-thiduathang', {
        ...
        title: {
            text: `Tỉ lệ xếp loại<br> thi đua tháng <br>cán bộ`,
            align: 'center',
            verticalAlign: 'middle',
            y: 0,
            x: -128,
     ...
     <div id="container2">
         ...
         <table>
             <tr style="font-weight: bold; font-size: 14px; ">
                 ...
                 <td style="text-align:center;width: 25%">Đội công t</td>
             </tr>   
             ${dataPrint}  
         </table>
     </div>
...
function xeploaithiduanam(title, data) {
...
$(document).ready(function() {
    ...
    $('#print-thiduathang').click(function() {
        print(`${tenphong} thi đua tháng xếp loại cờ xanh, cờ vàng ${title}`, dataPrint)
    });
    ...
    $('#btn-search-thiduanam').click(function() {
        ...
        $.ajax({
            ...
            data: { id },
            success: function(data) {
                dangkithiduaChart(`Biểu đồ tỉ lệ đăng kí thi đua năm năm ${nam} ${tenphong}`, data)
                xeploaithiduanam(`Biểu đồ tỉ lệ xếp loại thi đua năm ${nam}  ${tenphong}`, data)
            }
        })
    });
    ...
    $('#print-thiduanam').click(function() {
        printThiduanam(`thi đua năm ${tenphong} xếp loại  ${titleThiduanam}`, dataThiduanamPrint)
    });
    ...
    $('#print-dangvien').click(function() {
        printDangvien(`${tenphong} xếp loại  ${titleDangvien}`, dataDangvienPrint)
    });
    ...
    $('#btn-search-time').click(function() {
        ...
        let max = $('#time-to').val();
        $.ajax({
            url: `/conganhungyen/thiduakhenthuong/thongke/khenthuong/${id}`,
            data: { min, max },
            success: function(data) {
    ...
    $('#btn-search-time-ki-luat').click(function() {
        ...
        $.ajax({
            url: `/conganhungyen/thiduakhenthuong/thongke/kiluat/${id}`,
            data: { min, max },
            success: function(data) {
                $('#kiluat-list').html('')
